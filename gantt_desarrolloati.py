...
try:
    ...
    df = pd.read_csv(sheet_url, dayfirst=True, encoding='utf-8')
...
def actualizar_grafico(mes, estado, theme):
    ...
    df_filtrado['RN_trunc'] = pd.Categorical(df_filtrado['RN_trunc'], categories=df_filtrado['RN_trunc'], ordered=True)
